...
- Grid and table views, filtering (status, type, rating, source, tags, dates), and a "Pick for me" random picker for your Planned list
...
2. Copy `.env.example` to `.env` and fill in the required backend values: `DATABASE_URL`, `BETTER_AUTH_SECRET` (32+ random characters), `BETTER_AUTH_URL`, `FRONTEND_URL`, `TMDB_API_READ_ACCESS_TOKEN`, and the IGDB credentials. Open Library needs no API key; `OPEN_LIBRARY_CONTACT_EMAIL` is optional and identifies regular API traffic. `MOBILE_FRONTEND_URL` is optional for a deployed Capacitor host, while `DEV_TRUSTED_ORIGIN` is only for development. Production frontend builds require `VITE_API_URL`; `VITE_AUTH_URL` and `VITE_PUBLIC_APP_URL` are optional overrides, and `VITE_DEV_API_PROXY` is a development-only proxy override. In Docker builds, `VITE_API_URL`, `VITE_AUTH_URL`, and `VITE_PUBLIC_APP_URL` are baked in as build arguments rather than runtime environment variables. Semantic embeddings run locally with MiniLM; the model is downloaded and cached on the first embedding run.
...
| `pnpm frontend:test` / `pnpm backend:test`   | Run one test suite                         |
...
| `pnpm frontend:cap:sync`                     | Sync the Android Capacitor project         |
| `pnpm frontend:cap:open`                     | Open the Android project in Android Studio |
